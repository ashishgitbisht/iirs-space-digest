...
# ✅ Auto-email function (ENABLE AFTER GMAIL SETUP)
def send_iirs_newsletter():
    """Send HTML newsletter to Gmail"""
    msg = MIMEMultipart()
    msg['From'] = 'ashshbsht@gmail.com'  # ✅ YOUR GMAIL
    msg['To'] = 'ashshbsht@gmail.com'    # ✅ YOUR GMAIL
    msg['Subject'] = f'🚀 IIRS Daily Space Digest | {timestamp}'

    # Email body
    msg.attach(MIMEText(f"""
Dear IIRS Team,

Daily space technology digest ready ({len(news_digest)} updates):
• ESA, NASA, SpaceX mission news
• Satellite launches & Earth observation
• Space research breakthroughs

Generated by IIRS Library | ISRO Dehradun
    """, 'plain'))

    # Attach HTML newsletter
    with open(filename, 'rb') as f:
        ...
        part.add_header(
            'Content-Disposition',
            f'attachment; filename=IIRS_Space_News_{timestamp}.html'
        )
        msg.attach(part)

    # Gmail SMTP (UPDATE YOUR APP PASSWORD)
    try:
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()
        server.login('ashshbsht@gmail.com', 'xedtnclnglflwnpz')  # ✅ YOUR 16-CHAR APP PASSWORD
        server.send_message(msg)
        server.quit()
        print("✅ EMAILED to Gmail successfully!")
    except Exception as e:
        print(f"⚠️ Email failed: {e}")
        print("💡 Enable 2FA → Generate App Password → Update code")

# Uncomment this line after Gmail App Password setup:
# send_iirs_newsletter()

print("🎉 IIRS Daily Digest COMPLETE!")
print("📅 Next run: Check Tasks tab for schedule")
